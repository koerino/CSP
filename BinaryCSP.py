...
def consistent(assignment, csp, var, value):
    ...
    for c in csp.binaryConstraints:
        if c.affects(var) and value == assignment.assignedValues[c.otherVariable(var)]:
            return False
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
    ...
    v = selectVariableMethod(assignment, csp)
    if v != None:
        for val in orderValuesMethod(assignment, csp, v):
            if consistent(assignment, csp, v, val):
                inferences = noInferences(assignment, csp, v, val)
                if inferences != None:
                    assignment.assignedValues[v] = val
                    res = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod)
                    if res != None:
                        return res
                    for key, value in inferences:
                        assignment.varDomains[key].add(value)

                assignment.assignedValues[v] = None
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    minVars = []
    keys = domains.keys()
    minVal = len(domains) + 1
    for i in range(len(keys)):
        if assignment.assignedValues[keys[i]] == None:
            if len(domains[keys[i]]) < minVal:
                # reset min variables
                minVars = []
                minVal = len(domains[keys[i]])
                minVars.append(keys[i])

            elif len(domains[keys[i]]) == minVal:
                minVars.append(keys[i])
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    for value in values:
        ...
        for c in csp.binaryConstraints:
            if c.affects(var):
                v = c.otherVariable(var)
                if value in list(assignment.varDomains[v]):
                    count += 1
        tuples.append((value, count))

    for tuple in sorted(tuples, key=lambda tup: tup[1]):
        res.append(tuple[0])
...
def forwardChecking(assignment, csp, var, value):
    ...
    keys = []
    for c in csp.binaryConstraints:
        if c.affects(var) and assignment.assignedValues[c.otherVariable(var)] == None:
            key = c.otherVariable(var)
            keys.append(key)
            if value in list(domains[key]):
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    ...
    if v != None:
        for val in orderValuesMethod(assignment, csp, v):
            if consistent(assignment, csp, v, val):
                inferences = inferenceMethod(assignment, csp, v, val)
                if inferences != None:
                    assignment.assignedValues[v] = val
                    res = recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)
                    if res != None:
                        return res
                    for key, value in inferences:
                        assignment.varDomains[key].add(value)
...
def revise(assignment, csp, var1, var2, constraint):
    ...
    if constraint.affects(var1) and constraint.affects(var2):
        for value in assignment.varDomains[var2]:
            satisfied = False
            for v in assignment.varDomains[var1]:
        ...
        for key, val in inferences:
            assignment.varDomains[key].remove(val)
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    for c in csp.binaryConstraints:
        if c.affects(var) and assignment.assignedValues[c.otherVariable(var)] == None:
            queue.append((var, c.otherVariable(var), c))

    while len(queue) != 0:
        tuple = queue.pop()
        reviseInferences = revise(assignment, csp, tuple[0], tuple[1], tuple[2])
        if reviseInferences == None:
            for inference in inferences:
                assignment.varDomains[inference[0]].add(inference[1])
            return None

        if len(reviseInferences) != 0:
            for c in csp.binaryConstraints:
                if c.affects(tuple[1]):
                    queue.append((tuple[1], c.otherVariable(tuple[1]), c))
...
def AC3(assignment, csp):
    ...
    for c in csp.binaryConstraints:
        for key in csp.varDomains.keys():
            if c.affects(key): # invalid assignment should fail here
                queue.append((key, c.otherVariable(key), c))
